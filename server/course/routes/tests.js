...
router.post('/updateSectionTests', async function (req, res) {
    ...
    const tests_code = req.body.codes || ''
    ...
    try {
        ...
        const tests = await TestsModel.write(section_id, tests_code)
...
router.get('/getSectionTests', async function (req, res) {
    ...
    try {
        ...
        /* Quirks Notice: 
            `req.uid === -1` means that request is from internal service,
             so no need for checking permisson
        */
